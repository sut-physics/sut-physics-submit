...
> อัปเดตล่าสุด: **2026-07-23 12:30** — **deploy เสร็จครบทุกขั้นแล้ว (1-6) ระบบใช้งานได้จริง**

---

# 🚀 สรุปสำหรับ session ใหม่ (อ่านแค่นี้ก็พอ)

**ระบบขึ้นออนไลน์แล้ว ใช้งานได้จริง มี backup อัตโนมัติแล้ว** — ขั้น deploy 1-6 ปิดจบหมด

| | |
|---|---|
| เว็บ | **https://submit.tail42c76d.ts.net/** (ทดสอบจากมือถือ 5G นอกมหาวิทยาลัยแล้ว ✓) |
| Admin UI | https://submit.tail42c76d.ts.net/_/ (superuser — ดู `note.md`) |
| VM | `ssh -i ~/Downloads/ssh-key-2026-07-22.key ubuntu@161.118.215.176` |
| Backup | ตี 3 VM สร้าง snapshot → ตี 4 NAS ดึงลง `/volume2/submit-backups/` (ทดสอบดึง+กู้จริงผ่านแล้ว ✓) |
| บัญชีในระบบ | มีคนเดียว: `santa` (admin, approved) |

### ⏭️ สิ่งที่ต้องทำต่อใน session หน้า

**คุณ (santa) ทำเอง — ผมทำแทนไม่ได้**
1. **ย้ายรหัส PocketBase superuser** จาก `note.md` เข้า password manager แล้วลบบรรทัดนั้นทิ้ง
...
**รอตัดสินใจ / สั่งได้เลย**

4. **ลองใช้งานจริงรอบใหญ่ผ่านหน้าเว็บ** ← **แนะนำทำอันนี้ก่อน**
   ที่ทดสอบไปทั้งหมดเป็นการยิง API ตรง **ยังไม่เคยมีใครกดใช้ผ่าน UI จริงเลยสักครั้ง**
   (ฟอร์มส่งงาน, ปุ่มเปลี่ยนสถานะ, กระทู้ตอบกลับ, realtime 2 หน้าจอ, ตัวนับคิว, สีเตือน deadline)
   → สมัครบัญชีที่สองใน Incognito แล้วบอก Claude ให้ approve ให้
5. ~~เข้ารหัส backup~~ ✅ **เสร็จแล้ว 23 ก.ค.** (ดูหัวข้อด้านล่าง)

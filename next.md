...
> อัปเดตล่าสุด: **2026-07-23 20:20** — ระบบใช้งานได้เต็มรูปแบบ · **ล้างข้อมูลทดสอบหมดแล้ว รอเทสต์ใหม่ 24 ก.ค.**
...
**1. เช็ค cron รอบแรกที่รันเอง** ← สำคัญสุด ยังไม่เคยมีรอบไหนที่ไม่มีคนสั่ง
```bash
set -a; . ./.env; set +a
ssh -i "$VM_SSH_KEY" "$VM_USER@$VM_HOST" 'tail -20 /var/log/submit-snapshot.log; ls -la /srv/submit-backup/db /srv/submit-backup/storage'
```
ฝั่ง NAS ดู `pull.log` — ถ้าดึงสำเร็จจะเห็นโฟลเดอร์ย่อยตามชื่อคนส่งใน `storage/`
> ⚠️ ถ้า `storage` ที่ NAS ว่างทั้งที่ VM มีไฟล์ ให้เช็ค bind mount ก่อน (ดูกับดักใน OPERATIONS.md ข้อ 4)
